...
fn literal<'a>() -> Parser<'a, u8, Literal> {
    ...
    let b = (seq(KEYWORD_TRUE) | seq(KEYWORD_FALSE)).map(|s| Literal::Boolean(s == KEYWORD_TRUE));
    (n | b | s).name("literal")
}
...
pub fn parse_str<'a>(map_str: &'a str) -> Result<MapSpec, ParseError> {
    let ctx = new_context();
    push_scope(&ctx);
